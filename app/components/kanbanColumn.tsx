...
const KanbanColumn: React.FC<KanbanColumnProps> = ({
  ...
  const handleAddTask = () => {
    ...
    const status = title === 'To Do' ? 'todo' : title === 'In Progress' ? 'in-progress' : 'done';
  ...
  return (
    <div className='w-80 relative'>
      <div className='flex py-6 px-4 justify-between sticky top-0 bg-[#F4F6F8]'>
        {isEditing ? (
          <Input
            autoFocus
            value={newTitle}
        ...
        <div className='flex justify-center items-center'>
          <div>
            <DropdownMenu>
              <DropdownMenuTrigger className='p-2 bg-transparent rounded-full hover:bg-slate-200 transition duration-200'>
                <BsThreeDots />
              </DropdownMenuTrigger>
